...
def request_api(addr, data, userData, reqm):
    ...
    if addr == '/listAll' and reqm == "GET":
        ...
        if len(split[1]) == 36:
            ...
            if len(split) < 3:  # this is organizations/uuid
                if details is None:
                    return False
                    # if details["EntityType"] != "organization": # << make it so i can use this method for everything and just specify type here
                    #     return False;
            elif len(split) == 3:
                if split[2] == "departments":
                    ent_id_parent =
                    details = load_owned_departments()
            for key, value in details.iteritems():
                element.update({str(key): str(value)})  # DUMP ALL DETAILS
