    ...
    if addr == '/listAll' and reqm == "GET":
        ...
        if len(split) < 2:
            if split[0] == "organizations":
                data.append(load_all(split[0][:-1]))
                if len(data) > 0:
            ...
            if len(split[1]) == 36:
                ...
                if len(split) < 3:
                    ...
                    if split[2] == "departments":
                        ...
                        data.append(load_owned_departments(ent_id_parent))
                    elif split[2] == "vdcs":
                        ent_id_parent = details["id"]
                        print ent_id_parent
                        data.append(load_owned_departments(ent_id_parent))
