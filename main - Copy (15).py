...
def get_token_json_response(un, pa):  # TODO WHY CANT I LOG IN AS DEPARTMENT USER
...
def authorization_object_check_bot_up(obj_ent_id, user_acl_id, uuid_to_compare_to):
    obj = cloudDB.get_row_dict("tblEntities", {"id": obj_ent_id})
    if obj["UniqueId"] in uuid_to_compare_to
        return True
    if obj["id"] == 1
        return False
    ...
    authorization_object_check_bot_up(parent_ent_id, user_acl_id, uuid_to_compare_to)
    ...
    if addr == '/listAll' and reqm == "GET":
        ...
        if aclRole == "IT":
            ...
            if len(split) < 2:  # listing
                ...
                if len(split[1]) == 36:
                    if not authorized_to_see_object(entity_parent_id=aclID, enttype=split[0][:-1],
                                                    provided_uuid=split[
                                                        1]): authorization_object_check_bot_up  # make sure you actually own this
