...
def request_api(addr, data, userData, reqm):
    ...
    if addr == '/listAll' and reqm == "GET":
        ...
        if aclRole == "IT":
            ...
            myVDCS = cloudDB.get_multiple_row("tblEntities", "id='%s'" % aclID)
            resultantData = []
            for oneVDC in myVDCS:
                data = []
                vdcID = oneVDC["id"]
                if len(split) < 2:  # listing
                    ...
                    if split[0] == "vdcs":
                        ...
                        object_type = "slice_attached_network"
                        data.append(load_owned(dept_id, object_type))
                    elif split[0] == "image-libraries":
                        object_type = "imagelibrary"
                        data.append(load_all_available(vdc_id=vdcID, enttype=object_type))
                    elif split[0] == "virtual-networks":
                        object_type = "virtual_network"
                        data.append(load_all_available_vir_nets(vdcID))
                    else:
                        return False
                    if len(data) > 0:
                        resultantData += data
                    ...
                    if len(split[1]) == 36:
                        details = load_ent_details(uuid=split[1])
                        if not details["id"] == vdcID:  # TODO POTENTIALLY WONT WORK IN EDGE CASES
                            # make sure you actually own this
                            log.critical("ATTEMPT TO ACCESS ILLEGAL OBJECT")
                            print "ATTEMPT TO ACCESS ILLEGAL OBJECT"
                            return "Forbidden"
