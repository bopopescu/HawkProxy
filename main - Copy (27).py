...
def generate_options(obj_type, obj_uuid, data, details, action="create", child_details=None):  # parent details
    ...
    if obj_type == "subnets":
        if action == "create":
            options = {"entitytype": "switch_network_service", "name": data["name"], "servicetype": "networkSwitch",
                       "parententityid": details["id"]}
    elif obj_type == "nats":
        if action == "create":
            options = {"entitytype": "nat_network_service", "name": data["name"], "servicetype": "nat",
                       "parententityid": details["id"]}
            ...
            options = {
                ...
                "name": data["name"],
                "vdc_status": "Ready",
                "throughputinc": data["throughputinc"],
                # "maxinstancescount":1,
                # "begininstancescount":1,
                "throughput": data["throughput"],
                # "slicepreferencepolicy":"VDC overrides device",
                # "loadbalancertype":"Routed",
                # "firewalltype":"Routed",
                ...
                # "vlantype":"MAC-IN-MAC",
                # "qos":"Default",
                "nat_pat_mode": data["pat_mode"],
                "cooldown_up": data["cooldown_add"],
                "cooldown_down": data["cooldown_remove"],
                "throughput_red": data["throughput_red"],
                "throughput_green": data["throughput_green"],
                "ram_red": data["ram_red"],
                "ram_green": data["ram_green"],
                "throughputsArray": [
                    {
                        "name": "100",
                        "id": "100"
                ...
                "northbound_port": data["northbound"],
                "nat_address_type": data["nat_address_type"],
                "nat_static_address": data["nat_static_address"]
            }
