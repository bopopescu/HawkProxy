...
class bashtasklog
  ...
  func setQuiet
  func setTimestamp
  func setTimestampLong
  func setLogfile
  func printStatus
  func printStatusQuiet  
  var timestamp
  var timestampLong
  var logfile
  var quiet
...
bashtasklog::bashtasklog() {
  ...
  OPTARG=""
  OPTIND=1
  while getopts ":tqfl:w:" OPT; do
    case $OPT in
      t) setTimestamp 1 ;;
      q) setQuiet 1 ;;
      l) setLogfile $OPTARG ;;
      f) setTimestampLong 1 ;;
      w) setWidth $OPTARG ;;
      :)
    esac
  done
...
bashtasklog::printOkQuiet() {
  printStatusQuiet "1" "$1"
  return 0
}
<<BTL
NAME
