...
'''
s = xmlrpclib.ServerProxy('http://localhost:8000', allow_none=True)
tap = {"entitytype": "tap_network_service",  "parententityid": 29, "network_interface" : 69}
tapdbid=0
response = json.loads(s.genericPhptoPy("entity", 0, "create", json.dumps(tap)))
if "dbid" in response:
    tapdbid = response["dbid"]
print "Create tap ....", response
sys.exit()
'''

slice = {"name": "labSlice", "url": "http://cloudflow.dyndns.biz:8220",
         "description": "first slice @ http://cloudflow.dyndns.biz:8200"}
slice2 = {"name": "labSlice2", "url": "http://192.168.227.2:8225", "description": "Slice @ http://192.168.227.2:8220"}
...
print "Reget Slice1",
bulkCommands = {"functions": [{"entity": "slice", "dbid": sdbid, "type": "status", "options": {}}]}
print json.loads(s.genericPhptoPy("entity", 0, "multiple_functions", json.dumps(bulkCommands)))
...
organization = {"name": "xyz333", "description": "first test organization - before update", "email": "a@b.com",
...
department1 = {"name": "bDept", "description": "2nd test department - before update", "email": "a@b.com",
               "DefaultSliceEntityId": sdbid,
               "ParentEntityName": "westcoast",
               "resources": [
                   {"catagory": "compute", "type": "default", "cpu": 10, "ram": 20, "network": 200},
...
organization1 = {"name": "xyz3333", "description": "first test organization - after", "email": "abc@bom.com",
                 "DefaultSliceEntityId": 0, "location": "lahore",
                 "resources": [
                     {"catagory": "compute", "type": "default", "cpu": 10, "ram": 20, "network": 200},
                     {"catagory": "storage", "type": "gold", "capacity": 22, "iops": 333, "network": 200},
                     {"catagory": "network", "type": "vpn", "throughput": 220},
...
print "Update org....", s.genericPhptoPy("organization", odbid, "update", json.dumps(organization1))
...
vdbid = 0
vdc1 = {"name": None, "description": "first test vdc", "parententityid": ddbid,
...
vdc1u = {"name": "vdc2", "description": "first test vdcUPDATED", "parententityid": ddbid}
print "Update VDC....", s.genericPhptoPy("vdc", vdbid, "update", json.dumps(vdc1u))

print "Create User Group....",
ugdbid = 0
sgrp = {"entitytype": "user_group", "name": "ussg1", "parententityid": odbid}
...
print "Create User....",
uugdbid = 0
sgrp = {"entitytype": "user", "name": "user",
        "parententityid": ugdbid, "loginid": "admin2", "password": "admin1234",
        "email": "a@b.com",
        "useripaddress": "192.168.228.20",
        "aclrole": "organization", "acl_dbids": [odbid]
        }
response = json.loads(s.genericPhptoPy("entity", 0, "create", json.dumps(sgrp)))
if "dbid" in response:
...
organization = {"name": "westcoast", "description": "first test organization - after updated",
                "resources": [
                    {"catagory": "compute", "type": "default", "cpu": 55, "ram": 55, "network": 555},
                    {"catagory": "storage", "type": "gold", "capacity": 55, "iops": 555, "network": 555},
                    {"catagory": "network", "type": "vpn", "throughput": 555},
                    {"catagory": "network", "type": "nat", "throughput": 555}
                ]
                }
print "Update Organization", s.genericPhptoPy("organization", odbid, "update", json.dumps(organization))
...
ddbid1 = 0
print "Create Department1...",
response = json.loads(s.genericPhptoPy("department", 0, "create", json.dumps(department1)))
print response
if "dbid" in response:
    ddbid1 = response["dbid"]

vn = {"functions": [{"entity": "entity", "dbid": 0, "type": "create",
                     "options": {"entitytype": "virtual_network", "name": "network1", "description": "first network",
                                 "parententityid": odbid,
                                 "throughput": 100, "networktype": "shared",
                                 "attached_entities": [
                                     {"entitytype": "departments", "entities": [{"attachedentityid": ddbid},
                                                                                {"attachedentityid": ddbid1}]}]}},
                    #        {"entity":"entity","dbid":"35","type":"update","options":{"entitytype":"compute_network_service","attached_entities":{"entitytype":"serverfarm","entities":[{"attachedentityid":130}]}}}
                    ]}

response = json.loads(s.genericPhptoPy("entity", 0, "multiple_functions", json.dumps(vn)))
if "dbid" in response:
...
wigbid = 0
widget = {"parententityid": vdbid, "entitytype": "widger", "entitytype": "widget", "entitysubtype": "system",
          "serviceentityid": 44, "portentityid": 22, "childentityid": 66}
print "Create widget....",
response = json.loads(s.genericPhptoPy("entity", 0, "create", json.dumps(widget)))
print response
if "dbid" in response:
    wigbid = response["dbid"]

print "Update widget....", s.genericPhptoPy("entity", wigbid, "update", json.dumps({"childentityid": 88}))

print "delete widget....", s.genericPhptoPy("entity", wigbid, "delete", None)
...
response = json.loads(
    s.genericPhptoPy("entity", 0, "update", json.dumps({"parententityid": vdbid, "networkservicename": "nat1"})))
print "update nat....", response
...
interface0 = {"entitytype": "network_interface", "parententityid": vdbid,
              "interfaceindex": 0, "inteface_type": "regular",
              "beginserviceentityname": "switch1", "endserviceentityname": "nat1",
              }

int0dbid = 0
response = json.loads(s.genericPhptoPy("entity", 0, "create", json.dumps(interface0)))
if "dbid" in response:
    int0dbid = response["dbid"]
print "Create Interface0....", response
...
# print "Command deprovision VDC", json.loads(s.genericPhptoPy("entity", vdbid, "command", json.dumps({"command": "deprovision"})))


# print "Command Validate VDC", json.loads(s.genericPhptoPy("entity", vdbid, "command", json.dumps({"command": "validate"})))
# time.sleep(10)

# print "Command provision VDC", json.loads(s.genericPhptoPy("entity", vdbid, "command", json.dumps({"command": "provision"})))
